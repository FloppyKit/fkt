/* fkt_bip39.c - BIP39 English wordlist + validation, C89 */
#include "fkt_bip39.h"
...
int fkt_bip39_word_index(const char *word) {
    int lo = 0, hi = 2047, mid, cmp;
    if (!word || !word[0]) return -1;
    while (lo <= hi) {
        mid = lo + ((hi - lo) >> 1);
        cmp = strcmp(word, fkt_bip39_wordlist[mid]);
        if (cmp == 0) return mid;
...
static void append_bits(uint8_t *bits, int *bit_len, unsigned val, int nbits) {
    ...
    for (i = nbits - 1; i >= 0; i--) {
        int byte = (*bit_len) >> 3;
        int shift = 7 - ((*bit_len) & 7);
        if (val & (1u << i)) bits[byte] |= (uint8_t)(1u << shift);
        (*bit_len)++;
    }
}
...
int fkt_bip39_validate_checksum(const char words[][FKT_BIP39_WORD_BUF], int num_words) {
    ...
    uint8_t hash[32];
    int bit_len = 0;
    int ent_bits, cs_bits;
    int i, idx;
    if (num_words != 12 && num_words != 24) return 0;
    memset(bits, 0, sizeof(bits));
    for (i = 0; i < num_words; i++) {
        idx = fkt_bip39_word_index(words[i]);
        if (idx < 0) return 0;
        append_bits(bits, &bit_len, (unsigned)idx, 11);
    }
    ent_bits = (num_words * 11) - (num_words / 3);
    cs_bits = num_words / 3;
    fkt_sha256(bits, (size_t)(ent_bits >> 3), hash);
    for (i = 0; i < cs_bits; i++) {
        int ent_bit = ent_bits + i;
        int cs_val = (hash[0] >> (7 - i)) & 1;
        int mn_bit = (bits[ent_bit >> 3] >> (7 - (ent_bit & 7))) & 1;
        if (cs_val != mn_bit) return 0;
    }
    return 1;
}
